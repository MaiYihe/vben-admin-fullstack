...
public interface IAuthService {
    /**
     * 去数据库中，根据用户名查询加密密码；TODO (sql 数据库读操作)
     *  用户存在，继续
     *  用户不存在，抛出异常，异常返回
     * 比对查询密码和请求密码
     *  密码匹配：生成双 token
     ...
     *  密码不匹配：抛出异常，异常返回
     * @param request
     * @return JWT tokens（双 token）
     */
    TokenVO login(LoginRequest request);

    /**
     * 去数据库中，根据用户名查询用户是否存在 TODO (sql 数据库读操作)
     *  用户存在：抛出异常，异常返回
     *  用户不存在：
     *  - 将密码加密，把用户和密码一起写入数据库 TODO（sql 数据库写操作）
     *  - 生成 JWT(双 Token),写入内存 TODO (Redis 操作)
     *  
     *  - jti 写入 Redis
     *  - 正常返回双 token
     * @param request
     ...
     *  如果正确：继续
     *  如果不正确：抛出异常
     * 校验 jti 的 Redis 状态 TODO (Redis 操作)
     *  如果 jti 存在：继续
     ...
     * 解析 token，拿到 jti
     * 根据 jti，手动吊销 Redis 当中的 RefreshToken TODO (Redis 操作)
     *  如果删除成功：继续
    ...
    /**
     * TODO 用途：返回当前登录用户的权限码（permission codes / access codes）列表
     * 根据 userId 查询得到角色->权限 TODO（sql 数据库操作）
     * @return
     */
